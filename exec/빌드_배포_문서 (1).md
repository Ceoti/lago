...
SPRING_DATASOURCE_URL=jdbc:postgresql://<DB_HOST>:5432/stock_db
...
#### KIS API 인증
```bash
APP_KEY_A=<KIS_APP_KEY_A>
APP_SECRET_A=<KIS_APP_SECRET_A>
APP_KEY_B=<KIS_APP_KEY_B>
APP_SECRET_B=<KIS_APP_SECRET_B>
```
...
### 4.1 데이터베이스 접속 정보
- **호스트**: `<DB_HOST>` (환경변수로 설정)
- **포트**: 5432
- **데이터베이스명**: stock_db
- **사용자명**: `<DB_USER>` (환경변수로 설정)
...
#### 데이터베이스 계정
- **운영 계정**: 환경변수(`DB_USER` / `DB_PASSWORD`)로 관리
- **권한**: 읽기/쓰기 전체 권한
- **용도**: 애플리케이션 데이터 접근

#### KIS API 계정
- **UserA/UserB**: 환경변수(`APP_KEY_A`, `APP_KEY_B`)로 관리
