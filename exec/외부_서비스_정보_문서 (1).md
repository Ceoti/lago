...
- **프로젝트명**: LAGO 주식 시뮬레이션
- **클라이언트 ID**: `<GOOGLE_CLIENT_ID>` (환경변수로 관리)
- **클라이언트 시크릿**: `<GOOGLE_CLIENT_SECRET>` (환경변수로 관리)
...
- **클라이언트 ID (REST API 키)**: `<KAKAO_CLIENT_ID>` (환경변수로 관리)
- **클라이언트 시크릿**: `<KAKAO_CLIENT_SECRET>` (환경변수로 관리)
...
- **계정 A**:
  - App Key: `<KIS_APP_KEY_A>` (환경변수로 관리)
  - App Secret: `<KIS_APP_SECRET_A>` (환경변수로 관리)
- **계정 B**:
  - App Key: `<KIS_APP_KEY_B>` (환경변수로 관리)
  - App Secret: `<KIS_APP_SECRET_B>` (환경변수로 관리)
